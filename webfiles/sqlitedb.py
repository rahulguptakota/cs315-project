...
def setTime(selected_time):
    query_string = 'UPDATE TIME SET currtime = "3"'
    results = query(query_string)
    # alternatively: return results[0]['currenttime']
    return results[0].currtime # TODO: update this as well to match the
                                  # column name
...
def searchAuction(item_id, category, item_description, price, status):
    # TODO: rewrite this method to catch the Exception in case `result' is empty
    query_string = 'select itemID from ITEMS, CATEGORY where ITEMS.item_ID = $itemID AND ITEMS.item_ID = CATEGORY.item_ID AND ITEMS.currently = $price AND CATEGORY.name = $category AND ITEMS.endTime'
    result = query(query_string, {'itemID': item_id})
    if (isResultEmpty(result)):
	print 'no results found'
    else:
